...
```{python, eval=FALSE, attr.source='.numberLines'}
Shell(f"apptainer exec {diamond_sif} diamond makedb")
```
...
```{python, eval=FALSE, attr.source='.numberLines'}
Shell(f"apptainer exec {diamond_sif} diamond blastp")
```
...
Calculate BSR
```{python, eval=FALSE, attr.source='.numberLines'}
df_raw = pd.read_csv(diamond_raw, sep="\t", header=None, names=columns.split(" "))
...
Some utility functions are provided to make simple figures with `Ploty`.
...
<details>
    <summary>Why is a protein sequence fasta insufficient to full utilize Bakta?</summary>
...
```{python, eval=FALSE, attr.source='.numberLines'}
Shell(f"apptainer exec {kofamscan_sif} kofamscan --help") # expect crash
```
...
`LIB/"outputs/bakta.full/assembly.hypotheticals.faa` instead. Pre-computed outputs are available at `LIB/"outputs/kofamscan/kos.out"`
```{python, eval=FALSE, attr.source='.numberLines'}
...
```{python, eval=FALSE, attr.source='.numberLines'}
prodigal_aa_no_stop = Path("./prodigal/orfs.no_stop.faa")
with open(prodigal_aa_no_stop, "w") as f:
...
or use the pre-computed results at (`{LIB}/"outputs/deepfri/`).
```{python, eval=FALSE, attr.source='.numberLines'}
prodigal_aa_no_stop = Path("./prodigal/orfs.no_stop.faa")
deepfri_out = Path("./deepfri")
Shell(f"""
mkdir -p {deepfri_out}
cp {prodigal_aa_no_stop} {deepfri_out}/
cd {deepfri_out}
apptainer exec -B ./:/ws -W /ws ../{deepfri_sif} \
    deepfri \
        --ont mf bp ec \
        --fasta_fn /ws/{prodigal_aa_no_stop.name} \
        --output_fn_prefix /ws/chicken
""")
```
...
```{python, eval=FALSE, attr.source='.numberLines'}
apptainer exec {proteinbert_sif} \
    pbert run \
        --threads {CPUS} \
        -i {prodigal_aa} \
        -o protein_bert
...
apptainer exec {gapseq_sif} \
    gapseq find -p {gapseq_p} -l KEGG -K {CPU} -O -f {out_dir} {aa_fasta} \
...
mkdir -p {out_dir}
apptainer exec {gapseq_sif} \
    gapseq find-transport -K {CPU} -f {out_dir} {aa_fasta} \
...
Shell(f"""
mag = 1
dfp = pd.read_csv(f"./gapseq/mag{mag}/mag{mag}-all-Pathways.tbl", sep="\t", comment="#")
